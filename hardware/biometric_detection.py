...
parser = argparse.ArgumentParser(description="MR60BHA2 live logger")
parser.add_argument("--raw", action="store_true",
                    help="use instantaneous frame values (no 2-sample average)")
...
# moving-average helper (2-sample window unless --raw)
def mov_avg(prev: float | None, cur: float) -> float:
    if args.raw or prev is None:
        return cur
    return (prev + cur) / 2.0
...
for _ in itertools.repeat(None):
    ...
    if m := RX_DIST.search(txt):
        ...
        # always print distance as soon as we get it (throttled)
        if time.time() - last_dist_print >= PRINT_EVERY:
            print({"ts": iso8601(time.time()),
                   "dist_cm": round(dist_avg, 1)})
            last_dist_print = time.time()
